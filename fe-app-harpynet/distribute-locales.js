...
const targetRoot = path.resolve(__dirname, '../luci-app-harpynet/po');
...
async function main() {
    ...
    for (const file of files) {
        ...
        if (file === 'harpynet.pot') {
            const potTarget = path.join(targetRoot, 'templates', 'harpynet.pot');
        ...
        const match = file.match(/^harpynet\.([a-zA-Z_]+)\.po$/);
        if (match) {
            const lang = match[1];
            const poTarget = path.join(targetRoot, lang, 'harpynet.po');
