...
function createSettingsContent(section) {
  ...
  o = section.option(
    ...
    _("Delay in milliseconds before reloading harpynet after interface UP"),
  );
  o.depends("enable_badwan_interface_monitoring", "1");
  o.default = "2000";
  o.rmempty = false;
  ...
  o = section.option(
    ...
    "Частота обновления подписки",
    "Как часто HarpyNet будет заново загружать подписку для обновления ключей доступа, серверов и информации о подписке",
  );
  o.value("3h", "Каждые 3 часа");
  o.value("12h", "Каждые 12 часов");
  o.value("1d", "Каждые 24 часа");
  o.value("3d", "Каждые 3 дня");
  o.default = "12h";
  o.rmempty = false;
  ...
  o.cfgvalue = function (section_id) {
    return uci.get("harpynet", section_id, "download_lists_via_proxy_section");
  };
  o.load = function () {
    const sections = this.map?.data?.state?.values?.harpynet ?? {};
