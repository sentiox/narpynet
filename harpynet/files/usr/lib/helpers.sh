...
get_device_hwid() {
    ...
    echo "Router-$(echo "$mac" | md5sum | cut -c1-16)"
}

get_device_model() {
    cat /tmp/sysinfo/model 2>/dev/null || echo "OpenWrt Device"
}

get_harpynet_user_agent() {
    echo "ClashMeta/1.19.24 HarpyNet/${HARPYNET_VERSION}/router"
}
...
download_to_file() {
    ...
    for attempt in $(seq 1 "$retries"); do
        ...
        if [ -n "$user_agent" ] && [ -n "$hwid" ] && command -v curl >/dev/null 2>&1; then
            ...
            if [ -n "$http_proxy_address" ]; then
                http_proxy="http://$http_proxy_address" https_proxy="http://$http_proxy_address" \
                    wget -q -T 25 -O "$filepath" "$url" && result=0
            else
                wget -q -T 25 -O "$filepath" "$url" && result=0
            fi
        fi
